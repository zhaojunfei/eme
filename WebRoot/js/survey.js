...
function addQuestion() {
	...
	div1.innerHTML += "<div class='ques'><span>Q"
			...
			+ quesNum
			+ "' class='input-long question-style' placeholder='问卷题目'></div><div id='sel"
			+ quesNum
			...
			+ quesNum
			+ "'><image id='"+quesNum+"' style='width:10px;height:10px;margin-left:10px;cursor:pointer;visibility:hidden'  src='img/addsel.png'  alt='添加' onclick='addSelector(this)' ><image class='image"
			+ quesNum
			+ "' name='image"
			+ quesNum
...
function addSelector(obj) {
	...
	div1.innerHTML = "<input type='radio' name='S"
			...
			+ "'><input type='text'  placeholder='选项'  class='ST"
			+ obj.id
			+ "'><image id='"+obj.id+"' style='width:10px;height:10px;margin-left:10px;cursor:pointer;visibility:hidden'  src='img/addsel.png'  alt='添加' onclick='addSelector(this)' ><image class='image"
			+ obj.id
			+ "' name='image"
			+ obj.id
