				...
				small_img"
													...
													<div>
														<input type='hidden'
															...
															type='hidden'
															id="QT<s:property value="%{#status.count}" />" value="<s:if test="#sq.type==1">1</s:if><s:if test="#sq.type==2">2</s:if>">
													</div>
											...
											<section>
											<div class="control-group">
												<div class="ques" id="ques<s:property value="%{#status.count}" />">
													<div onmouseover='showDel(this)' onmouseout='hideDel(this)'>
														<span id='Q"
				+ quesNum
				+ "'>Q<s:property value="%{#status.count}" />:</span><input type='text'
															id="quesname<s:property value="%{#status.count}" />"
															value="<s:property
												value="#sq.content" />" class='input-long question-style' placeholder='问卷题目'>
														<img id="<s:property value="%{#status.count}" />" class='small_img'
															src='img/delquest.gif' alt='删除'
															onclick='delQuestion(this)'>
													</div>
													<div id="sel<s:property value="%{#status.count}" />"
				></div>
													<div class="selector<s:property value="%{#status.count}" /> selector-style">
														...
														<input type='hidden' id="AST<s:property value="%{#status.count}" />"><input
															type='hidden' id="QT<s:property value="%{#status.count}" />" value="3">
													</div>
												</div>
												</div>
												</section>
								...
								<div class="submit_top">
									<input type="hidden" name="survey.surveyId"
										...
										value="survey.surveyId" />">
									<span>问卷有效时间：</span> 
									<input type="date" name="survey.startTime" id="startTime" value="<s:property value="survey.startTime"/>" />至
