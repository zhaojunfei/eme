								...
								<tbody>
									<s:iterator value="suPageBean.list" var="s">
										<tr>
											...
											<td><a href="Student_Survey_Information_selectSurveyInfo?surveyId=<s:property value="#s.surveyId" />">填写</a></td>
										</tr>
									</s:iterator>
								</tbody>
							</table>
				...
				if (data.suPageBean.list.length == 0) {
					...
					$.each(data.suPageBean.list, function(i, value) {
						...
						var end = value.endTime.substr(0, value.endTime
								.indexOf('T'));
						$("#surveyList").append(
								"<tr><td>" + value.title + "</td><td>" + start
										+ "</td><td>" + end + "</td><td>"
										+ value.sponsor
										+ "</td><td><a href='Student_Survey_Information_selectSurveyInfo?surveyId="+value.surveyId+"'>填写</a></td></tr>");
					});
				}
